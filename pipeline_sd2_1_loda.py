...
class LODA_StableDiffusion2_1Pipeline(StableDiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        # shape for add latent
        new_shape = (latents.shape[0], latents.shape[1], latents.shape[2] // 4, latents.shape[3] // 4)
            ...
            for i, t in enumerate(timesteps):
                with torch.enable_grad():
                        ...
                        if i < LODA_steps:
                            ...
                            for step in range(inner_steps):
                                ...
                                kl_loss = self._compute_harmonic_mean_kl_raw(attention_store=attention_store,
                                                                             ...
                                                                             sigma=sigma,
                                                                             t = threshold)
                                # kl_loss = self._compute_harmonic_mean_jsd(attention_store=attention_store,
                                #                                           indices_to_alter=token_idx,
                                #                                           kernel_size = kernel_size,
                                #                                           sigma=sigma,
                                #                                           t=threshold)
                                # kl_loss = self._compute_harmonic_mean_bh(attention_store=attention_store,
                                #                   indices_to_alter=token_idx,
                                #                   kernel_size = kernel_size,
                                #                   sigma = sigma)
                    ...
                    if is_AFG:
                        ...
                        if i == AFG_steps:
                            print("Applying AFG")
                            self.apply_attention_masks(attention_store,
                                                       tokens=token_idx,
                                                       sigma = sigma_m,
                                                       p = p,
                                                       m =m,
