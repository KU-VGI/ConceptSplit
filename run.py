...
from pipeline_attend_and_excite_2 import AttendAndExcitePipeline
from utils import ptp_utils_LODA, vis_utils
from utils.ptp_utils_LODA import AttentionStore
...
def run_on_prompt(prompt: List[str],
                  ...
                  m = -1e8,
                  sigma_m = 1.5,
                  percentile = 90) -> Image.Image:
    if controller is not None:
        ptp_utils_LODA.register_attention_control(model, controller)
    outputs = model(height=512,
                    width=512,
                    prompt=prompt,
                    attention_store=controller,
                    target_attention_store=target_controller,
